...
// 2nd arg maps container path -> volume mnt
func (node *ResolvedNode) ParseCmd(
	tp TypedParams, mounts map[string]ExecVolumeMnt,
	_ WorkflowIndex, _ JobConfig, _ map[ExecType]string,
	...
	for inputName, input := range node.Inputs {
		...
		if input.Source.NodeId != nil {
			...
			var cntPath string
			if input.Mount != nil {
				cntPath = input.Mount.ContainerPath
			} else {
				cntPath = volumeMnt.CntPath
			}
			if input.Kind == "file" || input.Kind == "directory" {
				volumes.PropagatedPaths[inputName] = PropagatedMnt{
					CntPath: cntPath,
					MntSrc: volumeMnt,
				}
			}
			...
			if input.Mount != nil {
				mnt := *input.Mount
				if input.Kind == "file" || input.Kind == "directory" {
					volumes.ExplicitMnts[inputName] = VolumeMnt{
                        HostPath: input.Source.Path,
                        CntPath: mnt.ContainerPath,
                    }
				}
			}
		}
...
func (wf *ResolvedWorkflow) PropagateArgTypes() error {
	return nil
}