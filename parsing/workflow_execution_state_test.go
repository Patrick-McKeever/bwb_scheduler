...
// It is relevant that when WorkflowExecutionState.getSuccParams is called
// twice to announce completion of a command from the same parameter set that
...
func TestNonRedundancy(t *testing.T) {
	...
	if err != nil {
		...
		if len(succsOf1[2]) > 0 {
			t.Fatalf("node 1 triggers 2 twice when invoked repeatedly; should only do so once")
		}
	}
}
