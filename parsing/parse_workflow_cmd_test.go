...
// In BWB, if a variable is cmd-subbed via _bwb{VAR} syntax, it will not be
// added as an arg or a flag later.
func TestCmdSubArgOmission(t *testing.T) {
	var node WorkflowNodeV0
	lValForTrue := true
	lValForFlag := "--flag"
	node.ArgTypes = map[string]WorkflowArgType{
	...
	cmds, err := ParseNodeCmdV0(node, tp)
	if err != nil {
	...
	if cmds[0].BaseCmd[0] != "command subText" {
		t.Fatalf("expected cmd \"command subText\", got \"%s\"", cmds[0].BaseCmd[0])
	}
...
func basicIterValidation(
	node WorkflowNodeV0, baseProps map[string]any,
