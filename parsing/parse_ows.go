...
func (node *BwbJsonWorkflowNode) UnmarshalJSON(data []byte) error {
    ...
    paramsInnerRaw, err := json.Marshal(raw.Parameters.PyReduce[4])
    if err != nil {
        return err
    }
    ...
    if err := json.Unmarshal(paramsInnerRaw, &tupleBlock); err != nil {
        return err
    }
    ...
    outputsInnerRaw, err := json.Marshal(raw.Outputs.PyReduce[4])
    if err != nil {
        return err
    }

    if err := json.Unmarshal(outputsInnerRaw, &tupleBlock); err != nil {
        return err
    }
...
func PropagateArgTypes(workflow *Workflow) error {
    ...
    for i := 0; i < len(layeredTopSort); i++ {
    ...
    for i := len(layeredTopSort) - 1; i >= 0; i-- {
        for _, nodeId := range layeredTopSort[i] {
            nodeCopy := workflow.Nodes[nodeId]
            for pname, outLink := range outLinks[nodeId] {
                ...
                if _, sinkHasArgType := nodeCopy.ArgTypes[pname]; !sinkHasArgType {
                    ...
                    nodeCopy.ArgTypes[pname] = sinkArgType
                }
            }
            workflow.Nodes[nodeId] = nodeCopy
        }
